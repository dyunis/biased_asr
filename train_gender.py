...
def train(args, jsons, spk2genders):
    ...
    gender_optimizer=torch.optim.Adam(model.parameters(), lr=args.learning_rate, 
                                 betas=(0.9, 0.98))
    model.to(device)
    ...
    for epoch in range(args.n_epochs):
        ...
        for data in tqdm.tqdm(bucket_train_loader):
            gender_id=torch.tensor([idx_2_labels[train_set.utt2gender[gen_id]] 
                                for gen_id in data['utt_id']])
            gender_Y=torch.zeros(gender_id.shape[0],2,dtype=torch.long,device=device)
            for i in range(0,gender_Y.shape[0]):
                                gender_Y[i,gender_id[i]]=1
                    
            optimizer.zero_grad()
            gender_optimizer.zero_grad()
            
            log_probs,gender_log_probs, embed = model(data['feat'].cuda())
            loss = ctc_loss(log_probs.transpose(0, 1), data['label'].cuda(), 
                            data['feat_lens'].cuda(), data['label_lens'].cuda())
            gen_loss=gender_loss(gender_log_probs,(gender_Y).cuda())
...
#             gradient clipping does slowdown, value taken from ESPnet
            ...
            gender_losses.append(float(gen_loss))

        train_loss.append(np.mean(losses))
        train_gender_loss.append(float(gen_loss))
        gender_losses,losses, cer, wer = [], [], [],[]
        model.eval()
        for data in bucket_dev_loader:
            gender_id=torch.tensor([idx_2_labels[dev_set.utt2gender[gen_id]] 
                          for gen_id in data['utt_id']])
            gender_Y=torch.zeros(gender_id.shape[0],2,dtype=torch.long,device=device)
            for i in range(0,gender_Y.shape[0]):
                gender_Y[i,gender_id[i]]=1
        ...
        dev_gen_loss.append(np.mean(gender_losses))
...
#         log a single female and male utterance prediction vs label
        ...
        logging.info(f'training loss gender: {train_gender_loss[-1]}')
        logging.info(f'dev loss gender: {dev_gen_loss[-1]}')
    ...
    make_epoch_plot(dev_wer, 'dev WER', os.path.join(args.temp_root,
                                                     args.model_dir,
                                                     'dev_wer.png'))
    make_epoch_plot(train_gender_loss, 'train loss', os.path.join(args.temp_root,
                                                           args.model_dir,
                                                           'train_loss.png'))
    make_epoch_plot(dev_gender_loss, 'dev loss', os.path.join(args.temp_root,
                                                       args.model_dir,
                                                       'dev_loss.png'))
...
if __name__=='__main__':
    ...
    parser.add_argument('--temp_root',
                        ...
                        default='/scratch/asr_tmp_gender_loss2')
    ...
    parser.add_argument('--normalize',
                        ...
                        default=None)
    parser.add_argument('--merge_epoch',
                        type=int,
                        help='type of normalization [utt, spk, gndr] to use',
                        default=20)
